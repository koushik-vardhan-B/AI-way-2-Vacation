...
def delete_user_sessions(db: Session, user_id: int) -> int:
    ...
    deleted = db.query(models.Session)\
...
def delete_expired_sessions(db: Session) -> int:
    ...
    deleted = db.query(models.Session)\
        .filter(models.Session.expires_at < current_time)\
        .delete()
    db.commit()
    return deleted

# ===== Cleanup Functions =====
def delete_old_queries(db: Session, days: int = 90) -> int:
    """Delete queries older than specified days"""
    cutoff_date = datetime.utcnow() - timedelta(days=days)
    deleted = db.query(models.Query)\
        .filter(models.Query.created_at < cutoff_date)\
        .delete()
    db.commit()
    return deleted

def delete_old_api_usage(db: Session, days: int = 90) -> int:
    """Delete API usage records older than specified days"""
    cutoff_date = datetime.utcnow() - timedelta(days=days)
    deleted = db.query(models.ApiUsage)\
        .filter(models.ApiUsage.created_at < cutoff_date)\
        .delete()
    db.commit()
    return deleted